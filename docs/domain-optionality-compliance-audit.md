...
| Domain object | Status | Findings and required migration |
| --- | --- | --- |
| `Address` | Partial | Optional line 2 uses `Maybe<AddressLine>`. Required reference parameters are defensively checked despite non-nullable signatures. `WithAddressLine2` only sets a value; add `WithoutAddressLine2` so absence is explicit. |
