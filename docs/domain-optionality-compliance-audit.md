...
| `DateTimeRange` | Partial | Required `DateOnly` values are non-null and fallible calculations retain error reasons. `WithoutEnd` is a success-only result and uses `DateOnly.MaxValue` as an absence sentinel despite `End` being modeled as required. Either make end optional with `Maybe<DateOnly>` or rename the operation to reflect an open-ended sentinel explicitly. |
